...
describe('<HighlightOverlay/>', () => {
    ...
    function getRectsWithFill(component: React.ReactComponentElement, fillColor: string): Array<React.DOMElement> {
    ...
    it('sets mask ID based on page number', () => {
        ...
        expect(wrapper.getDOMNode().getElementsByTagName('mask')[0].id).equals('highlight-overlay-mask-345');
    });
    ...
    describe('pixel size', () => {
        it('matches the pixel size of the page [outer wrapper div]', () => {
            ...
            expectHeightWidth(wrapper.getDOMNode(), mockContext.pageSize.height, mockContext.pageSize.width);
        });
    ...
    describe('page scaling', () => {
        ...
        it('responds to page scaling [masked rect elements]', () => {
            ...
            expect(maskedRects.length).equals(2);
            expectHeightWidth(maskedRects[0],
                mockContext.pageSize.height * context.scale,
                mockContext.pageSize.width * context.scale);
            expectHeightWidth(maskedRects[1],
                mockContext.pageSize.height * context.scale,
                mockContext.pageSize.width * context.scale);
        });
    });
