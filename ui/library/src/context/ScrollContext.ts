...
import ScrollDetector, {
  ScrollDirection,
} from "../utils/ScrollDirectionDetector";
...
const EMPTY_NUMBER_TO_VISIBLE_ENTRY_DETAIL_MAP = Object.freeze(new Map<number, VisibleEntryDetailType>());
...
const DEFAULT_CONTEXT: IScrollContext = {
  ...
  isPageVisible: (opts) => {
    logProviderWarning(
      `isPageVisible(${JSON.stringify(opts)})`,
      "ScrollContext"
    );
    return false;
  },
...
export function useScrollContextProps(): IScrollContext {
  ...
  const [scrollThreshold, setScrollThreshold] =
  ...
  const [pageScrolledIntoViewThreshold, setPageScrolledIntoViewThreshold] =
    React.useState(DEFAULT_PAGE_SCROLLED_INTO_VIEW_THRESHOLD);
    ...
    const detector = new VisibleEntriesDetector<NodeDestination>({
      ...
      onVisibleEntriesChange: ({
        ...
        visibleEntries.map((entry) =>
          newEntries.set(
            entry.target.getAttribute(OUTLINE_ATTRIBUTE),
            {
    ...
    const detector = new VisibleEntriesDetector<number>({
      ...
      onVisibleEntriesChange: ({
        ...
        if(hiddenEntries.length) {
          ...
          const hiddenPageNums = hiddenEntries.map(e=>e.target?.getAttribute(PAGE_NUMBER_ATTRIBUTE) || "")
